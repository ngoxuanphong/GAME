...
def train_tam_thoi1(number_tran):
    ...
    for buoc3 in range(2*number_tran-1):
        list_player = [playerRandom1]*2 + [test1] + [random_player]*(getAgentSize()-3)
        kq, file_ = normal_main(list_player,1500, [0])
...
def train_tam_thoi2(number_tran):
    ...
    list_player= [playerScore2]*3 + [random_player]*(getAgentSize()-3)
    kq, file_2 = normal_main(list_player,3500, [0])
    matran_Win= file_2[0][np.where(file_2[1] == max(file_2[1]))[0][0]]
    np.save(f'{path_save_player}CK1_Win.npy',matran_Win)
        
    for buoc3 in range(2*number_tran-1):
        list_player = [playerRandom1]*2 + [test2] + [random_player]*(getAgentSize()-3)
        kq, file_ = normal_main(list_player,1500, [0])
        file_.append(file_2[0][np.where(file_2[1] == max(file_2[1]))[0][0]])
        np.save(f'{path_save_player}CK1_Matran.npy',file_)
        list_player= [playerScore2]*3 + [random_player]*(getAgentSize()-3)
...
def train_gen(number_tran, m):
    ...
    list_player= [playerScore3]*3 + [random_player]*(getAgentSize()-3)
    kq, file_2 = normal_main(list_player,3500, [0])
    matran_Win= file_2[0][np.where(file_2[1] == max(file_2[1]))[0][0]]
    ...
    for buoc3 in range(2*number_tran-1):
        gen_matrix(matrix1,matrix2)
        list_player = [playerScore3]*2 + [test_gen] + [random_player]*(getAgentSize()-3)
        kq, file_ = normal_main(list_player,1500, [0])
...
def test(play_state,file_temp,file_per):
    player = 'Khanh_130922' #Tên folder
...
# [playerRandom1]*2 + [test1] + [random_player]*(getAgentSize()-3)
    if path.exists(f'{path_save_player}CK_Win.npy') == False:
      train_tam_thoi1(3)
      train_tam_thoi2(3)
      list_player= [test1,test2] + [random_player]*(getAgentSize()-2)
      ...
      train_tam_thoi1(3)
      train_tam_thoi2(3)
      list_player= [test1,test2,test] + [random_player]*(getAgentSize()-3)
      kq, file_2 = normal_main(list_player,1500, [0])
      if kq[0]>kq[2] and kq[0]>kq[1]:
        ...
        while l == 0: 
          ...
          train_tam_thoi1(3)
          train_tam_thoi2(3)
          list_player= [test1,test2,test,test_gen] + [random_player]*(getAgentSize()-4)
          kq, file_2 = normal_main(list_player,2000, [0])
