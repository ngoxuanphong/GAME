...
player = 'Dat_200922'  #Tên folder của người chơi
...
class GeneticAlgorithm:
    ...
    class Agent:
        ...
        def __init__(self):
            if np.random.rand() >= 0.5:
                self.weights1 = np.random.rand(LEN_STATE,512)*3 - 1.5
                self.weights2 = np.random.rand(512,AMOUNT_ACTION)*2-1
            else:
                self.weights1 = np.random.choice(np.arange(-1.5,1.5,0.25),size=(LEN_STATE,512))
                self.weights2 = np.random.choice(np.arange(-1,1,0.25),size=(512,AMOUNT_ACTION))
    ...
    def choose_better_Agent(self,Agent1,Agent2) -> Agent:
        list_player = [Agent1.play,Agent2.play] + [self.agent_random] * (getAgentSize()-2)
    ...
    def train(self,num_generations = 10,num_agents = 64):
        ...
        for num in range(num_generations):
            if num==0 and self.best_agent==0 and self.best_agent2==0:
                ...
                self.reset()
                lst1 = [self.best_agent.play,self.best_agent2.play]+ [self.agent_random]*(getAgentSize()-2)
                ...
                if rank[1]>rank[0]+50:
                    ...
                    pass
                try:
                    lst2 = [self.best.play,self.best_agent.play]+ [self.agent_random]*(getAgentSize()-2)
