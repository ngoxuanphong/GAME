...
def p_train_lan_2(n):
    ...
    for change in range(n):
        for phong_id_model_choose in range(amount_model):
            ...
            matrix_remove = (np.random.random((p_len_state, getActionSize()))*2 - 1)*0.05
            ...
            list_player = [p_test_matrix_old] + [p_test_matrix_new] + [player_random]*int(getAgentSize()-2)
            ...
            if count[1] - count[0] >= 20 and count[1] > 480:
                ...
                # list_player = [test] + [player_random]*int(getAgentSize()-1)
