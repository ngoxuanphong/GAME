...
def step1(ten):
    lp = [create0] * getAgentSize()
    kq,file = normal_main(lp,getAgentSize()**2,[0])
    per = file[0]
    # print(len(per))
...
def screen0(popu,ten):
    step = len(popu)//(getAgentSize() -1)
    amount = getAgentSize() -1 
    for game in range(step):
        per = popu[game*amount:(game+1)*amount]
        pools = [test,test0,test1,test2,test3,test4]
        lp = pools[:getAgentSize()]
        try:
            kq,_ = normal_main(lp,getAgentSize()* (getAgentSize()+1) + 1,per)
...
def step2(ten):
    for id in range(getActionSize()):
        ...
        popu = []
        for act in range(getActionSize()):
...
def step3():
    for id in range(1,getAgentSize()):
