...
def GetScoreEndGame(env):
    for id_score in range(4):
        ...
        score_civ_yellow = env[e_o_idp + 39]*env[e_o_idp + 1] + env[e_o_idp + 40]*env[e_o_idp + 4] + env[e_o_idp + 41]*np.sum(env[e_o_idp + 9:e_o_idp + 12]) + env[e_o_idp + 42]*env[e_o_idp + 2]
...
def getValidActions(p_state):
    ...
    if phase == 0: #Chọn ô đặt người
        ...
        list_action[14:18] = (people_in_warehouse[3:7] < 7) #Gỗ, bạc, gạch, vàng
    ...
    if phase == 0: #Đặt người
       ...
       ##print('Đặt', action, 'người')
        ...
        if action in np.arange(29, 37):
            ...
            raise 'Action không nằm trong phase này'
        ...
        if id_warehouse == 0: #Thêm lúa
            ...
            if np.sum(env[e_idp+12:e_idp+15]) > 0 or (env[e_idp+9:e_idp+12] > 0).any(): #Nếu còn thẻ công cụ để dùng
                ...
                env[e_idp + 31 + id_warehouse] = 0 #Không có công cụ thì roll luôn
                env = RollDice(count_dice, env, id_warehouse, e_idp, 0)

        elif id_warehouse in [8,9,10,11]: #Lấy thẻ civ
            env[E_PULL_RECENT] = id_warehouse
            if np.sum(env[e_idp+5:e_idp+9]) >= (id_warehouse - 6):
                env[E_PHASE:] = 0  #Đổi phase
                env[E_PHASE + 3] = 1
       ...
       ##print('trong phase 3', check_end_pull_people)
        if np.sum(env[77:81]) == count_res_give: #Khi đủ nguyên liệu thì mở thẻ civ mới, lấy thẻ cho người chơi, sang người chơi khác
           ...
           ##print('Vẫn trả được')
            ...
            env[e_idp + 5 + 7] += env[card_civ_id + 5] #Thêm nguyên liệu bạc
            env[e_idp + 5 + 8] += env[card_civ_id + 6] #Thêm nguyên liệu vàng

            if env[card_civ_id] == 0:
                env[e_idp + 5 + 6] += env[card_civ_id + 4] #Thêm nguyên liệu gạch
            ...
            if env[card_civ_id] > 0: #Thẻ roll 2 xúc xắc để lấy tài nguyên
                if env[card_civ_id + 4] > 0: #Gỗ
                    ...
                    res_take = 6
                env = RollDice(2, env, res_take, e_idp, 0) 
            ...
            if env[card_civ_id + 1] == 1: #Thẻ chọn thêm 1 thẻ civ mới từ chồng bài úp
        ...
        if action == 0:
            ...
            total_tool = env[E_ALL_TOOL]
            id_warehouse = int(env[E_PULL_RECENT])
            count_dice = int(env[e_idp+31+id_warehouse])
            env[37:40] = np.abs(env[37:40]) #Trả lại công cụ như bình thường
            env[e_idp + 31 + id_warehouse] = 0 
            env = RollDice(count_dice, env, id_warehouse, e_idp, total_tool)

            ###Nếu còn có thể lấy người thì lấy, không thì sang người chơi khác
        elif action in [37, 38, 39]:
            id_action = action - 37
               ...
               ##print('Xong 1 vòng')
                ...
                for id_return in range(4):
                    ...
                    if env[e_id_return + 21] == 0: #Chưa trả thức ăn
                        if env[e_id_return + 3] >= env[e_id_return + 2]:
                            env[e_id_return + 3] -= env[e_id_return + 2]
                            env[e_id_return + 21] = 1
                        elif (np.sum(env[e_id_return + 5: e_id_return + 9]) + env[e_id_return + 3]) < env[e_id_return + 2]:
                            env[e_id_return] -= 10 #Trừ điểm
                            env[e_id_return + 3] = 0
                            env[e_id_return + 21] = 1 
                        else:
                            env[81] = env[e_id_return + 2] - env[e_id_return + 3]
                            env[e_id_return + 3] = 0
                ...
                ###print('Đổi người chơi khác lấy người', env[E_START_CIV:E_END_BUILD])
                env[(idp+1)%4] = 1
...
one_game([test, test, test, test], [0])