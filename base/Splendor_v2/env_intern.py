...
def one_game_print(list_player, *print_mode):
...
def normal_main_print(list_player, num_game=1, print_mode=False):
    ...
    for _n in range(num_game):
        ...
        lst_player_new = [list_player[p_lst_idx[i]] for i in range(4)]

        winner = one_game_print(lst_player_new, print_mode)
...
def random_player(p_state):
    ...
    return np.random.choice(arr_action)
