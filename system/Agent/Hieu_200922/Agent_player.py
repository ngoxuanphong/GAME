...
def get_random_matrix():
    ...
    list_player = [Hieu_player1]*(amount_player() - 3) + [random_player]*3        #đấu ngẫu nhiên, lấy các bot thắng random
...
def train(x):
    ...
    list_player = [Hieu_player_2]*(amount_player()-1) + [random_player]
...
def DEA(best_matrix):
    ...
    if len(all_new_matrix) > len(all_matrix):
        ...
        all_so_tran = np.zeros(len(all_matrix))
        all_ratio = np.zeros(len(all_matrix))
        list_player = [Hieu_player_2]*(amount_player()-2) + [random_player, random_player]
...
def cross_matrix(all_index, best_matrix):
    ...
    for loop in range(5):
        ...
        list_player = [random_player]*(amount_player()-3) + [Hieu_player_2, Hieu_player_2, Hieu_player_DEA] 
        result_cr, data_save_cr = normal_main(list_player,len(all_matrix), [0]) 
        if np.argmax(result_cr) == amount_player()-1:
            all_new_matrix.append(trial_matrix)

def choose_better_matrix(best_matrix, best_matrix_new):
    global trial_matrix
    trial_matrix = best_matrix_new
    result = np.zeros(amount_player())
    list_player = [Hieu_player_DEA, test, Hieu_player_2] + [random_player]*(amount_player()-3)
    ...
    list_player = [Hieu_player_DEA, random_player, random_player]  + [random_player]*(amount_player()-3)
    result_cr, data_save_cr = normal_main(list_player, 1000, [0]) 
    result[0] += result_cr[0]
    # print('new_bot_random', result_cr)

    list_player = [random_player, test, random_player]  + [random_player]*(amount_player()-3) 
    result_cr, data_save_cr = normal_main(list_player, 1000, [0]) 
    result[1] += result_cr[1]
