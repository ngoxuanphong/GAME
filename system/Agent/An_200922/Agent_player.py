...
def generate_sg(p_state, temp_file, per_file):
    ...
    check_win = check_victory(p_state)
    if check_win != -1:
        temp_file.pop(0)
        if check_win == 1:
            for i in temp_file:
...
def generate_bnn(p_state, temp_file, per_file):
    if type(per_file) == list and len(per_file) == 0:
        per_file = np.full((len(p_state), amount_action()), 0.5)
...
def train(num_cycle=1):
    ...
    try:
        ...
        per_file = [np.zeros(amount_action()), np.full(amount_action(), 0.01)]
    
    list_player = [generate_sg] * amount_player()
    ...
    try:
        ...
        per_file = np.full(amount_action(), 0.5)
    
    list_player = [generate_g1] * amount_player()
    kq, per_file = normal_main(list_player, 1000, per_file)
    ...
    list_player = [generate_bnn] * amount_player()
    kq, per_file = normal_main(list_player, 1000, per_file)
    ...
    for _ in range(num_cycle):
        ...
        #########################
        list_player = [generate_fnn] + [cur_best_bot] + [Ann_rdb] * (amount_player() - 2)
        kq, per_file = normal_main(list_player, 1000, per_file)

        list_player = [generate_fnn] + [test_sg] + [Ann_rdb] * (amount_player() - 2)
        kq, per_file = normal_main(list_player, 1000, per_file)

        list_player = [generate_fnn] + [test_g1] + [Ann_rdb] * (amount_player() - 2)
        kq, per_file = normal_main(list_player, 1000, per_file)

        list_player = [generate_fnn] + [test_bnn] + [Ann_rdb] * (amount_player() - 2)
        kq, per_file = normal_main(list_player, 1000, per_file)

        list_player = [generate_fnn] + [Ann_rdb] * (amount_player() - 1)
        kq, per_file = normal_main(list_player, 1000, per_file)
        ...
        list_player = [scoring] * (amount_player()-1) + [Ann_rdb]
        ...
        list_player = [test_data] + [Ann_rdb] * (amount_player()-1)
