...
def Hieu_player1(state, file_temp, file_per):
    ...
    if len(file_temp) < 2:
        file_temp = [0, 0, 0]
        raw1 = len(state)
        col1 = min(amount_action(), len(state))
        raw2 = col1
...
def train(x):
    ...
    random1 = 2000
    n_game -= random1
    list_player = [Hieu_player1]*(amount_player() - 2) + [random_player]*2        #đấu ngẫu nhiên, lấy các bot thắng random
    ...
    temp_data_save_loop = []
    while n_game > 0:
        list_player = [Hieu_player1]*(amount_player() - 2) + [random_player, test]
